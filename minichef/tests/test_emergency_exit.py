...
def test_emergency_exit_with_profit(
    ...
    # withdraw and confirm we made money
    vault.withdraw({"from": whale})
    assert poolToken.balanceOf(whale) + donation >= startingWhale
...
def test_emergency_exit_with_no_gain_or_loss(
    ...
    to_send = poolToken.balanceOf(strategy)
    print("Balance of Vault", to_send)
    poolToken.transfer(gov, to_send, {"from": strategy})
    assert strategy.estimatedTotalAssets() == 0

    # have our whale send in exactly our debtOutstanding
    whale_to_give = vault.debtOutstanding(strategy)
    poolToken.transfer(strategy, whale_to_give, {"from": whale})
    ...
    vault.withdraw({"from": whale})
    assert poolToken.balanceOf(whale) + amount + whale_to_give >= startingWhale
