...
contract StrategyGenLevAAVE is BaseStrategyInitializable {
    ...
    IProtocolDataProvider private constant protocolDataProvider =
        IProtocolDataProvider(0x65285E9dfab318f57051ab2b139ccCf232945451);
    ...
    address private constant wrappedNative =
        0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    ...
    IUniLikeSwapRouter private PRIMARY_ROUTER =
        IUniLikeSwapRouter(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
    IUniLikeSwapRouter private SECONDARY_ROUTER =
        IUniLikeSwapRouter(0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106);
    ...
    function estimatedRewardsInWant() public view returns (uint256) {
        ...
        if (rewardToken == address(want)) {
            ...
            return
                tokenToWant(
                    rewardToken,
                    rewardTokenBalance.add(pendingRewards)
                );
        }
    }
        ...
        returns (address[] memory _path)
    {
        bool is_wrapped_native =
            _token_in == address(wrappedNative) ||
                _token_out == address(wrappedNative);
    ...
    function _sellRewardTokenForWant(uint256 amountIn, uint256 minOut)
        internal
    {
        if (amountIn == 0) {
            return;
